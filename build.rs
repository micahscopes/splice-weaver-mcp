...
fn main() {
    ...
    match extract_catalog_examples() {
        Ok(catalog) => {
            ...
            println!(
                "cargo:warning=Generated catalog.json with {} examples",
        ...
        Err(e) => {
            ...
            fs::write(
                &catalog_path,
                serde_json::to_string_pretty(&empty_catalog).unwrap(),
            )
...
fn extract_catalog_examples() -> Result<Value, Box<dyn std::error::Error>> {
    ...
    if !website_path.exists() {
        ...
        let output = std::process::Command::new("git")
            .args([
                "clone",
                "https://github.com/ast-grep/ast-grep.github.io.git",
            ])
            .arg(&website_path)
            .output()?;
    ...
    println!(
        "cargo:warning=Found ast-grep website at: {}",
        website_path.display()
    );
...
fn extract_catalog_example(
    ...
    let playground_link = playground_re
        .captures(content)
        .and_then(|caps| caps.get(1))
        .map(|m| m.as_str())
        .unwrap_or("")
    ...
    // Determine type
    let rule_type = if yaml_content.is_empty() {
        "Pattern"
    } else {
        "YAML"
    };

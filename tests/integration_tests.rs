...
async fn test_evaluation_client_full_workflow() -> Result<()> {
    ...
    assert_eq!(
        client.conversation_history.len(),
        0,
    ...
    assert_eq!(
        client.conversation_history.len(),
        0,
        "Should remain empty after reset"
    );
...
async fn test_default_test_cases_structure() -> Result<()> {
    ...
    for (i, test_case) in test_cases.iter().enumerate() {
        assert!(
            ...
            "Test case {} should have name",
            i
        );
        assert!(
            !test_case.prompt.is_empty(),
            "Test case {} should have prompt",
            i
        );
        assert!(
            !test_case.expected_tools.is_empty(),
            "Test case {} should expect tools",
            i
        );

        // Test success criteria function works
        let mock_result = splice_weaver_mcp::evaluation_client::EvaluationResult {

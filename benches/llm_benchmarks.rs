...
use splice_weaver_mcp::evaluation_client::{
    create_default_test_cases, EvaluationClient, EvaluationClientConfig, EvaluationResult,
};
...
fn benchmark_model_comparison(c: &mut Criterion) {
    ...
    let models = vec!["gpt-3.5-turbo", "gpt-4", "claude-3-sonnet", "llama-2-7b"];

    let test_prompt =
        "Find all function declarations in this JavaScript code: function test() { return 42; }";
        ...
        group.bench_with_input(
            ...
            |b, model_name| {
                b.iter(|| {
                    ...
                    let result = rt
                        .block_on(run_benchmark_suite(config, test_prompt))
                        .unwrap();

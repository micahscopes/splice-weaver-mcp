...
use mcp_ast_grep::evaluation_client::{EvaluationClient, EvaluationClientConfig, TestCase};
use mcp_ast_grep::snapshot_utils::{SnapshotManager, ExportFormat, cleanup_old_snapshots};
use std::fs;
...
async fn capture_snapshots(matches: &clap::ArgMatches) -> Result<()> {
    ...
    let test_cases = if let Some(test_name) = matches.get_one::<String>("test-name") {
        ...
        mcp_ast_grep::evaluation_client::create_default_test_cases()
    };
    ...
    for test_case in test_cases {
        ...
        match client.evaluate_prompt(&test_case.prompt).await {
            Ok(result) => {
                ...
                let metadata = mcp_ast_grep::evaluation_client::SnapshotMetadata {
                ...
                let snapshot = mcp_ast_grep::evaluation_client::ResponseSnapshot {
                    metadata,
...
async fn compare_snapshots(matches: &clap::ArgMatches) -> Result<()> {
    ...
    let baseline_days: u64 = matches.get_one::<String>("baseline-days")
        .unwrap()
        .parse()
        .unwrap_or(7);
    ...
    if regressions.is_empty() {
        ...
        for (i, regression) in regressions.iter().enumerate() {
            println!("\n{}. Test: {}", i + 1, regression.previous.metadata.test_name);
...
async fn cleanup_snapshots(matches: &clap::ArgMatches) -> Result<()> {
    ...
    info!("Cleanup completed. Would remove {} old snapshots", removed_count);
